...
export class TransitionSwitch extends React.Component {
    ...
    React.Children.map(props.children, child => child).forEach(child => {
      ...
      if (!found && match) {
        ...
        if (this.state.enteringRouteKey) {
          if (this.state.enteringRouteKey === child.key) return;
        }
        ...
        this.setState({
          ...this.state,
          leavingRouteKey:
            this.state.leavingRouteKey && !this.props.parallel
  ...
  render() {
    ...
    React.Children.map(this.props.children, child => child).forEach(child => {
      if (child.key === this.state.enteringRouteKey) {
        ...
        else if (child.props.render) component = child.props.render(props);
        else component = child.props.children;
  ...
  componentDidMount() {
    if (
      ...
      this.enteringRouteChildRef.componentWillAppear
    ) {
      this.enteringRouteChildRef.componentWillAppear(() =>
  ...
  componentDidUpdate(prevProps, prevState, prevContext = this.prevContext) {
    ...
    if (
      ...
      if (
        ...
        this.state.leavingRouteKey == null &&
        prevState.leavingRouteKey != null
      )
        this.enteringRouteChildRef.componentWillEnter(() =>
    ...
    if (
      ...
      prevMatch.isExact === match.isExact
    )
      return;
    ...
    if (
      ...
      if (this.props.parallel) {
        if (
          ...
          this.enteringRouteChildRef.sameComponentWillEnter(() =>
            this.enteringChildEntered()
          );
        } else {
          this.enteringRouteChildRef.componentWillEnter(() =>
    ...
    if (
      this.leavingRouteChildRef &&
      this.leavingRouteChildRef.componentWillLeave
    ) {
      if (
        this.leavingRouteChildRef &&
        (!prevState.leavingRouteKey || this.props.parallel)
      ) {
        this.leavingRouteChildRef.componentWillLeave(() =>
